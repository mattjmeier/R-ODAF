---
title: "Transcriptome Profiling of Bisphenols"
author: "Matthew J. Meier"
subtitle: "TempO-Seq Analysis: Bisphenol Study"
output:
  html_document:
    toc: true
    toc_float: true
...
Report prepared for: Marc Beal, Tara Barton-Mclaren  
Purpose of report:  

* Present results from TempO-Seq  
...
flag=FALSE
if (flag==TRUE) { load("Partial_analysis.RData") }
...
library('DESeq2')
library('edgeR')
library('org.Hs.eg.db') # MOUSE: library('org.Mm.eg.db') # HUMAN: library('org.Hs.eg.db') # RAT: library('org.Rn.eg.db')
library('enrichplot')
...
# Set file locations				
	sampledir <- "/home/mpiage/data/2020_bisphenols/"
  setwd(sampledir)
	outputdir <- paste(sampledir, "./DEG_output/", sep="")
	if(!dir.exists(outputdir)) {dir.create(outputdir)}
# Names of files to load
	SampleDataFile <- "./biospyder/count_table.csv" # "genes.data.tsv" #This tab delimited file contains the merged RSEM.genes.results files
...
# Specify which groups need to be compared 
	...
	short_contrast_names <- paste(contrasts$V1) # Customize these for your experiment... Must be short enough to fit as Excel tab titles.
...
# Set analysis ID. This ID will be used as prefix for the output files
	analysisID <-"2020_bisphenols"
# Specify used platform/technology for data generation:
	Platform <- "TempO-Seq" # Specify "RNA-Seq" or "TempO-seq"
...
sampleData <- sampleData[,(colSums(sampleData)>100000)] # 100k reads per sample for TempO-Seq
...
all(colnames(sampleData) %in% rownames(DESeqDesign)) # Sanity check: each sample name (column) in the count data should have a corresponding row in the metadata
...
contrasts <- contrasts %>% dplyr::filter(grepl("BPA_",V1))
...
ensembl <- useMart("ensembl", dataset = "hsapiens_gene_ensembl" ) #  Mouse: "mmusculus_gene_ensembl" # Human: "hsapiens_gene_ensembl" # Rat: 
  ...
  if (nrow(sigTab) == 0) {
    ...
    if (Platform=="TempO-Seq") {
      sigTab <- dplyr::left_join(sigTab,
                                 biospyder_annos,
                                 by=c("ensembl_gene_id"="PROBE_NAME"))
    } else {
      sigTab <- dplyr::left_join(sigTab,
                               id_table,
                               by="ensembl_gene_id")
...
PCAplot <- plotPCA(rld, intgroup = c("chemical","dose","group","batch","pool"), ntop=nrow(assay(rld)), returnData = TRUE)
data_pca <- plotPCA(rld, intgroup = c("chemical","dose","group","batch","pool"), ntop=nrow(assay(rld)), returnData = TRUE)
percentVar <- round(100 * attr(data_pca, "percentVar"))

ggplot(PCAplot, aes(PC1, PC2, color=factor(dose))) + 
  geom_point(size=1) +
 ...
 ggplot(PCAplot, aes(PC1, PC2, color=batch)) + 
  geom_point(size=1) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) +
  facet_wrap(~chemical) +
...
ggplot(PCAplot, aes(log(PC1+1), log(PC2+1), color=batch)) + 
  geom_point(size=1) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) +
  facet_wrap(~chemical) +
...
## SUBSET
keep <- grep(colData(rld)$chemical, pattern="cells", invert=T, ignore.case = T)
rld_sub <- rld[,keep]

PCAplot <- plotPCA(rld_sub,
                   intgroup = c("chemical","dose","group","batch","pool"),
                   ntop=nrow(assay(rld_sub)),
                   returnData = TRUE)
...
## Perform PCA analysis and make plot
#plotPCA(rld_degs, intgroup = c("chemical","dose","group","pool","batch"), ntop=nrow(rld_degs)) #+ theme(legend.position = "none")
PCAplotDEGs <- plotPCA(rld_degs, intgroup = c("chemical","dose","group","batch","pool"), ntop=nrow(assay(rld_degs)), returnData = TRUE)
...
ggplot(PCAplotDEGs, aes(log(PC1+1), log(PC2+1), color=batch)) + 
  geom_point(size=1) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) +
  facet_wrap(~chemical) +
...
ggplot(PCAplotDEGs, aes(PC1, PC2, color=as.factor(pool))) + 
  geom_point(size=1) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) +
  facet_wrap(~chemical) +
...
mat_top <- assay(rld)[row.names(assay(rld)) %in% allResultsOrdered_logFC_filter[1:nBest,]$ensembl_gene_id,]
mat_top <- mat - rowMeans(mat)
...
heatmap_df <- as.data.frame(colData(rld)[,c("chemical","dose","original_names")]) # Customize!
pheatmap(mat,
         annotation_col=heatmap_df,
         show_rownames = FALSE,
         border_color = NA,
...
# Genes of particular known interest: PGR (progesterone receptor), TFF1 (Trefoil factor 1)
gene1 <- plotCounts(dds,
                   gene="PGR_5102",
                   intgroup=c("dose","group"),
                   returnData = TRUE)
...
gene1 <- gene1 %>% dplyr::filter(grepl("BPA_",group))
gene2 <- gene2 %>% dplyr::filter(grepl("BPA_",group))

ggplot(gene1, aes(x = dose, y = count)) + 
      geom_point() + 
      ylab('Normalized count') +
      xlab('Dose') +
      ...
      theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
      ggtitle("PGR_5102") +
      scale_x_continuous(breaks=sort(unique(gene1$dose)), labels = function(x) round(as.numeric(x), digits=4)) #format(x, scientific = FALSE))

ggplot(gene2, aes(x = dose, y = count)) + 
      geom_point() + 
      ylab('Normalized count') +
      xlab('Dose') +
      coord_trans(x= "log10", y = "log10") +
...
enrich_go_all <- enrichGO(gene = ensemblDEGs,
                         ...
                         OrgDb = org.Hs.eg.db, # org.Mm.eg.db # org.Hs.eg.db # org.Rn.eg.db
                         keyType = "ENSEMBL",
                         ont = "all",
                         pAdjustMethod = "BH",
                         pvalueCutoff = 0.01,
...
mouseWiki <- rWikiPathways::downloadPathwayArchive(organism="Homo sapiens", format = "gmt") # Rattus norvegicus, Mus musculus, Homo sapiens
...
DEG_pathways <- clusterProfiler::enricher(
        ...
        TERM2GENE = wpid2gene,
        TERM2NAME = wpid2name)
DEG_pathways <- DOSE::setReadable(DEG_pathways, org.Hs.eg.db, keyType = "ENTREZID") # org.Mm.eg.db org.Hs.eg.db org.Rn.eg.db
...
for (i in 1:length(levels(significantResults$contrast))) {
  ...
  DEG_pathways <- clusterProfiler::enricher(
          ...
          TERM2GENE = wpid2gene,
          TERM2NAME = wpid2name)
  DEG_pathways <- DOSE::setReadable(DEG_pathways, org.Rn.eg.db, keyType = "ENTREZID")
