...
## Set libPaths
#.libPaths(c("~/shared/R/x86_64-pc-linux-gnu-library/3.6",.libPaths()))
...
# Set file locations				
sampledir <- "~/shared/data/2020_bisphenols/"
setwd(sampledir)
...
DESIGN <- "group"	#Column name which defines the groups to be compared
intgroup <- DESIGN # May want to make this include more columns instead, for plotting PCA etc.
...
heatmap_df <- dplyr::mutate(heatmap_df, dose=as.factor(dose))
row.names(heatmap_df) <- heatmap_df$original_names
# pheatmap(mat,
#          annotation_col=heatmap_df,
#          show_rownames = FALSE,
#          border_color = NA,
...
chems <- DESeqDesign[which(!DESeqDesign$chemical %in% c("Estradiol",
                                                        "No Cells; Brain +ve control",
                                                        "No cells; Universal human +ve control",
                                                        "cells, no treatment",
                                                        "Dex")),] %>%
  select(chemical) %>%
  distinct() %>%
...
pdf(file="./heatmaps.bisphenols.pdf")
for (chem in chems){

samples_for_heatmap <- DESeqDesign[which(DESeqDesign$chemical %in% c(chem, "DMSO 0.1%")),] %>%
  dplyr::arrange(dose) %>%
...
row.names(mat_subset_ordered) <- stringr::str_replace(string = row.names(mat_subset_ordered),
                                         pattern = "_.*",
                                         replacement = "")
genes_for_heatmap_subset[,2] <- stringr::str_replace(string = genes_for_heatmap_subset[,2],
                                         pattern = "_.*",
                                         replacement = "")
